...
<body class="bg-neutral-bg text-gray-800 antialiased">
            ...
            <header class="h-20 bg-white shadow-sm flex items-center justify-between px-6 z-10 gap-4">
                ...
                <div class="flex items-center gap-2 md:gap-4">
                    ...
                    <div x-data="{ open: false }" class="relative">
                        <button @click="open = !open" @click.away="open = false" class="relative p-2.5 text-gray-400 hover:text-accent hover:bg-orange-50 rounded-xl transition-all focus:outline-none">
                        ...
                        <div x-show="open" x-transition class="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-lg py-2 z-50 border border-gray-100" style="display: none;">
                            ...
                            <div class="max-h-80 overflow-y-auto">
                                @forelse($unreadNotifications as $notification)
                                    <a href="{{ $notification->link ?? '#' }}" class="block px-4 py-3 hover:bg-gray-50 border-b border-gray-50 transition-colors {{ $notification->type == 'danger' ? 'bg-red-50/50' : ($notification->type == 'success' ? 'bg-green-50/50' : '') }}">
                                        <p class="text-sm font-bold {{ $notification->type == 'danger' ? 'text-red-600' : ($notification->type == 'success' ? 'text-green-600' : 'text-gray-800') }}">{{ $notification->title }}</p>
    ...
    <div x-data="{ 
        ...
        }" class="fixed bottom-24 md:bottom-6 right-6 z-40">
        <!-- Chat Window -->
        <div x-show="chatOpen" 
