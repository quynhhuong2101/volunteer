...
<div class="max-w-3xl mx-auto py-10 px-4">
    ...
    <div class="mb-8">
        ...
        <p class="text-slate-500 mt-2">Sự kiện: <span class="font-bold text-indigo-600">{{ $event->title }}</span></p>
    </div>
    ...
    <form action="{{ route('student.events.register.store', $event->id) }}" method="POST" class="space-y-8">
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
            ...
            <div class="space-y-3">
                ...
                <label class="block relative">
                    <input type="radio" name="position_id" value="{{ $pos->id }}" class="peer sr-only" {{ $pos->remaining <= 0 ? 'disabled' : 'required' }}>
                    <div class="p-4 rounded-xl border-2 border-slate-100 cursor-pointer transition-all peer-checked:border-indigo-600 peer-checked:bg-indigo-50 hover:bg-slate-50 {{ $pos->remaining <= 0 ? 'opacity-60 cursor-not-allowed bg-slate-50' : '' }}">
                        <div class="flex justify-between items-center">
                            <div>
                                <h4 class="font-bold text-slate-700 peer-checked:text-indigo-700">{{ $pos->name }}</h4>
                                <p class="text-xs text-slate-500">{{ $pos->description }}</p>
                            </div>
                            <div class="text-right">
                                @if($pos->remaining <= 0)
                                    <span class="inline-block px-2 py-1 rounded-md bg-red-100 text-red-600 text-xs font-bold uppercase">Đã đủ</span>
                                @else
                                    <span class="inline-block px-2 py-1 rounded-md bg-emerald-100 text-emerald-600 text-xs font-bold uppercase">Còn {{ $pos->remaining }} chỗ</span>
                                @endif
                            </div>
                        </div>
                    </div>
        ...
        <div class="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 opacity-80">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <div class="font-bold text-slate-700">{{ auth()->user()->student_id ?? 'N/A' }}</div>
                </div>
                 <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase mb-1">Email</label>
                    <div class="font-bold text-slate-700">{{ auth()->user()->email }}</div>
                </div>
                 <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase mb-1">Số điện thoại</label>
                    <div class="font-bold text-slate-700">{{ auth()->user()->phone ?? 'N/A' }}</div>
                </div>
            </div>
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    @if($field->type == 'text' || $field->type == 'textarea')
                        <input type="text" name="field_{{$field->id}}" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-indigo-500" placeholder="Nhập câu trả lời của bạn..." {{ $field->is_required ? 'required' : '' }}>
                    
                    @elseif($field->type == 'select')
                        <select name="field_{{$field->id}}" class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 focus:outline-none focus:border-indigo-500" {{ $field->is_required ? 'required' : '' }}>
                    ...
                    @elseif($field->type == 'checkbox')
                        <div class="space-y-2">
                            ...
                            <label class="flex items-center space-x-3">
                                <input type="checkbox" name="field_{{$field->id}}[]" value="{{ $option }}" class="w-5 h-5 text-indigo-600 bg-slate-50 border-slate-300 rounded focus:ring-indigo-500">
        ...
        <div class="bg-blue-50 rounded-xl p-4 border border-blue-100 flex items-start gap-3">
             ...
             <div class="text-sm text-blue-800">
                 Tôi cam kết tham gia đầy đủ và tuân thủ quy định của BTC. Tôi hiểu rằng tôi chỉ có thể hủy đăng ký trong vòng <strong class="text-blue-900">3 tiếng</strong> sau khi gửi đơn.
             </div>
        </div>
